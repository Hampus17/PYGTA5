...
def processImage(inpImage):
    ...
    # white_lower = np.array([0, 140, 0])
    # white_upper = np.array([255, 255, 255])

    white_lower = np.array([np.round(0 / 2), np.round(0.90 * 255), np.round(0.00 * 255)])
    white_upper = np.array([np.round(360 / 2), np.round(1.00 * 255), np.round(0.30 * 255)])

    yellow_lower = np.array([np.round(40 / 2), np.round(0.00 * 255), np.round(0.40 * 255)], dtype=np.uint8)
    yellow_upper = np.array([np.round(60 / 2), np.round(1.00 * 255), np.round(1.00 * 255)], dtype=np.uint8)

    mask_yellow = cv2.inRange(hls, yellow_lower, yellow_upper)
    mask_white = cv2.inRange(hls, white_lower, white_upper)
    combined_mask = cv2.bitwise_or(mask_yellow, mask_white)
    hls_result = cv2.bitwise_and(inpImage, inpImage, mask=combined_mask)
...
def perspectiveWarp(inpImage):
    ...
    altdst = np.float32([(0, 0),
                         (inpImage.shape[1], 0),
                         (inpImage.shape[1], inpImage.shape[0]),
                         (0, inpImage.shape[0])])

    # (int(0.65 * width), int(0.35 * height)),
    # (int(0.35 * width), int(0.35 * height)),
    # (0, height),
    # (width, height),

    # Perspective points to be warped
    asrc = np.float32([[590, 440],
                      [690, 440],
                      [200, 640],
                      [1000, 640]])

    src = np.float32([[int(inpImage.shape[1] / 2 - 600), int(inpImage.shape[0] - 200)],
                       [int(inpImage.shape[1] / 2 - 300), int(0.50 * inpImage.shape[0])],
                       [int(inpImage.shape[1] / 2 + 300), int(0.50 * inpImage.shape[0])],
                       [int(inpImage.shape[1] / 2 + 600), int(inpImage.shape[0] - 200)]])

    # Window to be shown
    adst = np.float32([[200, 0],
                      [1200, 0],
                      [200, 710],
                      [1200, 710]])

    dst = np.float32([[0, inpImage.shape[0]],
                         [0, 0],
                         [inpImage.shape[1], 0],
                         [inpImage.shape[1], inpImage.shape[0]]])
